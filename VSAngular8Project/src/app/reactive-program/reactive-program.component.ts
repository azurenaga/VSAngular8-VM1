...
export class ReactiveProgramComponent implements OnInit{
  title = 'Reactive programming concept'; 
   ...
   ngOnInit() { 
      ...
      const observer1 = { 
        next: (num: number) => {this.filteredNumbers.push(num); this.val2 += num; }, 
        error: (err: any) => console.log(err), 
        complete: () => console.log("Observation completed") 
     }; 

     const observer2 = { 
      next: (num: number) => {this.processedNumbers.push(num); this.val3 += num; }, 
      error: (err: any) => console.log(err), 
      complete: () => console.log("Observation completed") 
   };
      ...
      // clickEvent$.subscribe( () => console.log(this.counter++ )); 
    }
}
